...
系统吸收 TradingAgents 中适合本项目的结构，但不复制它的交易执行方式：

1. 评分与技术、基本面、信息面、资金主线、大盘与全球环境分别形成结构化角色结论；
2. 每个角色必须给出状态、立场、置信度、支持证据、反证、缺失项和证据引用；
3. 多方与空方观点并列保留，单一利好或单一模型结论不能创建订单；
4. 独立风险委员会检查实盘总开关、券商连接、kill switch、评分溯源和人工确认；
5. 组合裁决只给出观察、回测、模拟或实盘预检查建议，始终保持 `order_capability=false`；
6. 相同证据生成稳定复核编号；首页自动刷新只读，用户主动联网复核时才按该编号覆盖写入审计表；
7. 复盘对照买入持有、对应宽基指数和策略历史版本，不把未来结果倒灌到历史时点。

可选外部模型只接收真实证据和上述确定性角色摘要。模型调用失败、证据缺失或输出引用无效时，系统保留规则复核并关闭模型结论，不影响统一风控和订单内核。
