# Quant Data Gateway V3.28 / Broker Onboarding and Adaptive Trading

## V3.28 券商接入、自适应评分与统一交易账本

V3.28 在既有 V3.27 数据映射和评分基础上补齐券商接入诊断与交易闭环：

- `/auto-trading` 集中展示筛选、行情详情、信息面、回测、实时模拟、真实交易、统一记录、数据中心以及 QMT/PTrade/同花顺/HTTP 桥状态；
- QMT、PTrade、同花顺和通用本地桥均提供逐项配置诊断；同花顺零售桌面端仅支持启动和委托提醒，自动下单必须使用官方或券商授权桥；
- 真实委托依次经过数据新鲜度、评分溯源、策略适配、风险网关、确认队列和券商路由，批准后重新检查，订单与券商原始返回统一审计；
- 短线、波段、长线、定投、核心卫星、事件驱动按不同证据和风控配置计算，市场弱势与数据质量下降会提高门槛或降低仓位；
- 信息面补充来源健康、去重、影响对象、传导链与原文链接；数据中心展示自动交易就绪度、缺失/过期字段和范围刷新结果；
- 交易记录只按真实成交汇总成交金额与费用，当前持仓按最新快照统计，避免委托、标注和历史快照重复计数；
- 移动端提醒支持钉钉、飞书、企业微信和通用 Webhook，默认关闭且不能代替订单人工确认。

完整配置、安全边界和接口说明见 `docs/V328_BROKER_AND_ADAPTIVE_SCORING.md`。

真实交易默认关闭。没有本地券商终端、SDK、账户授权、真实新鲜数据、风控通过和用户确认时，系统不会提交真实订单，也不会伪造账户、成交或资金数据。

## V3.27 Session-aware Global Risk / Trading UX

## V3.27 Session-aware Global Risk / Trading UX

V3.27 在不重写既有评分映射的前提下，补齐跨市场时段判断、相关证据去重和交易页面可读性：

- `/api/market/global-sentiment?symbol=300750` 先读取标的行业、主营和题材，再动态选择可追溯海外参照：半导体对应费城半导体，光伏对应太阳能ETF，锂电对应锂电产业ETF，银行对应金融ETF；无法可靠映射时只使用全球宽基，不强行套用行业指数；
- 支持医疗、生物科技、能源、油气、黄金、有色、工业、军工、运输、消费、公用事业、核能、房地产、互联网、人工智能等行业代理，并明确区分“行业基准”和“全球宽基背景”；
- 同属一个指数簇或资产家族的高相关证据不会被简单累加，行业代理设置权重上限，避免纳指、行业ETF和期货重复放大同一风险；
- 个股对应的全球行业背景最多占环境分的 15%，且必须有国内市场证据。数据过期、来源不足、行业无法映射或只有单一相关家族时不触发自动买入；
- 基本面不再只读取筛选行里的旧分数：系统会从本地公司画像读取不晚于决策时点的财务披露，记录报告日、来源、证据字段和缺失项；缺少披露日期或财务来源时仍保持缺失；
- 大盘情绪使用独立缓存的上证、深成、创业板、沪深300、科创50日线与市场宽度，避免上证指数代码与平安银行 `000001` 缓存串号；全球行业参照只在时段和新鲜度合格时有限调节；
- `/api/market/capital-evidence/{symbol}` 展示公开日资金流、全天及5/15/30/60分钟量价方向代理、上午/下午拆分和基金持仓披露。公开资金流不等于券商逐笔，量价代理不等于主动买卖盘，基金持仓不等于当前机构账户；
- 实时模拟与筛选读取同一份缓存和评分溯源；历史回测只使用决策时点前可得数据，绝不把当前海外行情回填到历史日期；
- `/live-trading` 新增多股票只读决策观察，显示买一卖一、评分、数据新鲜度、持仓成本和盈亏；没有真实券商账户时显示缺失原因，不伪造为零资金或零持仓；
- K 线缓存会校验请求数量是否完整，长周期请求不会再被短缓存截断；异常标注聚合限流且不参与价格轴缩放，成交量、MACD、KDJ 保持独立可见；
- 信息事件补充就业、央行、限售解禁、指数调仓、减持、回购、增发、供应扰动、汇率、地缘、出口管制、航运和网络安全等类型，未来事件只做观察和风险门禁，不预设结果方向。

规则、数据边界和验收说明见 `docs/V327_SESSION_AWARE_GLOBAL_RISK.md`。
