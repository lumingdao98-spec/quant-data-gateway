...
function scoreFromQuote(q,session){const chg=Number(q.change_pct||0),vr=Number(q.volume_ratio||1),turn=Number(q.turnover||0);return {technical_score:Number.isFinite(chg)?Math.max(20,Math.min(88,58+chg*3+Math.min(vr||1,3)*4)):null,information_score:null,fundamental_score:null,market_score:session.can_refresh?58:null,anomaly_features:{price_jump_pct:Math.abs(chg||0),volume_ratio:vr,turnover_rate:turn},score_source:'quote_snapshot_observation'}}
