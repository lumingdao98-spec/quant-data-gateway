...
<div class="app">
  ...
  <aside class="side">
    ...
    <div class="hint">休市时“手动tick”按纸面回放处理，只记录研究信号，不代表真实盘中实时成交。多只股票可在信号流右上方按标的单独查看。</div>
  </aside>
  <main class="main">
    <div class="cards">
    ...
    <div class="panel">
      ...
      <div class="panel-b"><div class="table-wrap"><table><thead><tr><th>时间</th><th>名称</th><th>代码</th><th>动作</th><th>总分</th><th>基本/技术/信息/大盘</th><th>异常</th><th>目标仓位</th><th>模式</th><th>原因</th></tr></thead><tbody id="signalRows"></tbody></table></div></div>
    </div>
...
<script>
const $=id=>document.getElementById(id);const esc=s=>String(s??'').replace(/[&<>]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[m]));const fmt=(v,d=2)=>v==null||Number.isNaN(Number(v))?'--':Number(v).toFixed(d);const money=v=>Number(v||0).toLocaleString('zh-CN',{maximumFractionDigits:2});const pct=v=>fmt(v,2)+'%';const cls=v=>Number(v)>=0?'up':'down';let selectedSymbol='all',lastSignals=[];function log(s){$('log').textContent=new Date().toLocaleTimeString()+' '+s+'\n'+$('log').textContent}
...
async function startEngine(){const js=await api('/api/realtime-paper/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({symbols:symbols(),initial_cash:Number($('cash').value||100000),interval_seconds:Number($('interval').value||15),horizon:$('horizon').value})});renderStatus(js);renderSymbolTabs();log('已启动实时模拟')}
async function stopEngine(){const js=await api('/api/realtime-paper/stop',{method:'POST'});renderStatus(js);log('已停止')}
async function manualTick(){const s=(selectedSymbol!=='all'?selectedSymbol:symbols()[0])||'300750',price=Number($('tickPrice').value||0);const js=await api('/api/realtime-paper/tick',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({symbol:s,price,last:price,technical_score:68,information_score:58,fundamental_score:60,market_score:55,evidence:['手动tick/纸面观察'],manual_replay:true,is_trading_session:false})});log('tick '+s+' '+(js.signal?.action||'--')+' · '+(js.signal?.session_mode||'--'));await refreshAll()}
function renderStatus(js){const st=js.state||js.data?.state||{};const p=js.portfolio||js.data||{};$('mStatus').textContent=st.status||'--';$('mEquity').textContent=money(p.equity);$('mCash').textContent=money(p.cash)+' / '+money(p.available_cash);$('mRet').textContent=pct(p.total_return_pct);$('mRet').className=cls(p.total_return_pct);$('mMv').textContent=money(p.market_value);$('mDay').textContent=money(p.daily_pnl);$('mDd').textContent=pct((p.max_drawdown||0)*100);$('mTrades').textContent=p.trade_count_today??'--';$('riskBox').innerHTML=`状态 ${esc(st.status||'--')}；交易时段 ${st.is_trading_session?'是':'否'}；新鲜度 ${esc(st.freshness_status||'--')}<br>paper_only=true；real_broker_connected=false<br><span class="warn">休市自动模拟不会伪装实时行情，可用手动tick/历史回放做研究观察。</span>`}
...
function renderSymbolTabs(){const syms=['all',...new Set([...symbols(),...lastSignals.map(x=>x.symbol).filter(Boolean)])];$('symbolTabs').innerHTML=syms.map(s=>`<button class="${selectedSymbol===s?'active':''}" onclick="selectedSymbol='${s}';renderSignals({data:lastSignals});renderSymbolTabs()">${s==='all'?'全部':esc(s)}</button>`).join('')}
function renderSignals(js){lastSignals=js.data||lastSignals||[];const rows=selectedSymbol==='all'?lastSignals:lastSignals.filter(x=>String(x.symbol)===String(selectedSymbol));$('signalRows').innerHTML=rows.map(x=>`<tr><td>${esc((x.timestamp||'').replace('T',' ').slice(11,19))}</td><td>${esc(x.name||'--')}</td><td>${esc(x.symbol)}</td><td>${esc(x.action)}</td><td>${fmt(x.final_score,1)}</td><td>${fmt(x.fundamental_score,1)} / ${fmt(x.technical_score,1)} / ${fmt(x.information_score,1)} / ${fmt(x.market_score,1)}</td><td>${fmt(x.anomaly_score,1)}</td><td>${pct((x.target_weight||0)*100)}</td><td>${esc(x.session_mode||'--')}</td><td>${esc(x.reason||'--')}<div class="mini">${esc((x.evidence||[]).slice(0,2).join('；'))}</div></td></tr>`).join('')||'<tr><td colspan="10" class="muted">暂无信号</td></tr>';renderSymbolTabs()}
function renderOrders(js){const all=js.data||[];const rows=selectedSymbol==='all'?all:all.filter(x=>String(x.symbol)===String(selectedSymbol));$('orderRows').innerHTML=rows.map(x=>`<tr><td>${esc((x.created_at||'').replace('T',' ').slice(11,19))}</td><td>${esc(x.symbol)}</td><td>${esc(x.side)}</td><td>${esc(x.status)}</td><td>${fmt(x.price)}</td><td>${x.quantity??0}</td><td>${x.filled_quantity??0}</td><td>${esc(x.reason||'--')}</td></tr>`).join('')||'<tr><td colspan="8" class="muted">暂无订单</td></tr>'}
function renderAudit(js){const rows=js.data||[];$('auditBox').innerHTML=rows.slice(0,12).map(x=>`<div>${esc((x.created_at||'').replace('T',' ').slice(11,19))} <b>${esc(x.event_type)}</b> ${esc(JSON.stringify(x.payload||{})).slice(0,160)}</div>`).join('')||'暂无审计'}
