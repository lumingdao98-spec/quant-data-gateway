...
<main>
  ...
  <section class="stack">
    ...
    <div class="panel">
      ...
      <div class="b"><div id="accountBox" class="notice">读取中...</div><div class="table-wrap" style="margin-top:10px"><table><thead><tr><th>代码</th><th>名称</th><th>数量</th><th>可用</th><th>成本</th><th>市价</th><th>市值</th><th>盈亏</th></tr></thead><tbody id="positionsRows"><tr><td colspan="8">读取中...</td></tr></tbody></table></div></div>
    </div>
    <div class="panel">
...
function renderPositions(js){
  ...
  $('positionsRows').innerHTML=rows.map(p=>`<tr><td>${esc(p.symbol)}</td><td>${esc(p.name||'--')}</td><td>${esc(p.quantity??p.volume??0)}</td><td>${esc(p.available_quantity??p.available??'--')}</td><td>${esc(p.cost_price??p.avg_price??'--')}</td><td>${esc(p.last_price??p.price??'--')}</td><td>${money(p.market_value??p.amount)}</td><td class="${cls(p.pnl??p.unrealized_pnl)}">${money(p.pnl??p.unrealized_pnl)}</td></tr>`).join('')||'<tr><td colspan="8">暂无持仓；若券商未连接则显示接口不支持/未授权。</td></tr>';
}
function normalizeRecordRows(orders,trades,records){
  const rows=[];
  (orders.data||[]).forEach(x=>rows.push({source:'委托',symbol:x.symbol,side:x.side||x.status,price:x.limit_price||x.price,qty:x.quantity,amount:x.amount,status:x.status||x.status_reason}));
  (trades.data||[]).forEach(x=>rows.push({source:'成交',symbol:x.symbol,side:x.side,price:x.price,qty:x.quantity,amount:x.amount||x.fee,status:x.filled_at||x.source}));
  (records.data||[]).slice(0,30).forEach(x=>rows.push({source:x.table||'记录',symbol:x.symbol,side:x.side||x.status||x.event_type,price:x.price||x.limit_price,qty:x.quantity,amount:x.amount||x.pnl||x.realized_pnl,status:x.status_reason||x.reason||x.event_type}));
