...
function renderTradeDrawer(d){const events=tradeEvents(d),cost=d.cost_summary||{},pos=d.position_summary||{},p=btPeriod(d),params=d.params||{},metrics=d.metrics||{};$('tradeDrawerSummary').innerHTML=`<div><span>标的</span><b>${esc(d.name||d.symbol)} ${esc(d.symbol)}</b></div><div><span>回测区间</span><b style="font-size:15px">${esc(p.start)} 至 ${esc(p.end)}</b></div><div><span>策略收益</span><b class="${cls(d.total_return_pct)}">${pct(d.total_return_pct)}</b></div><div><span>闭合交易</span><b>${esc(d.trade_count||0)}笔</b></div><div><span>交易流水</span><b>${esc(d.trade_event_count||events.length)}条</b></div><div><span>仓位模式</span><b>${esc(params.position_sizing||'score_weighted')}</b></div><div><span>交易周期</span><b>${esc(params.horizon||'swing')}</b></div><div><span>复利</span><b>${params.compound_returns===false?'关闭':'开启'}</b></div><div><span>总成本</span><b>${money(cost.total_cost||0)}</b></div><div><span>成交额</span><b>${money(cost.turnover||0)}</b></div><div><span>平均成本/股</span><b>${esc(pos.avg_cost_basis||cost.avg_cost_basis||'--')}</b></div><div><span>最大持仓</span><b>${esc(pos.max_shares||cost.max_position_shares||0)}股</b></div><div><span>期末现金</span><b>${money(pos.cash||d.final_equity)}</b></div><div><span>期末持仓</span><b>${esc(pos.shares||0)}股</b></div><div><span>期望/赔率</span><b style="font-size:15px">${esc(metrics.expectancy??'--')} / ${esc(metrics.payoff_ratio??'--')}</b></div>`;$('tradeDrawerRows').innerHTML=events.map(tradeDrawerRow).join('')||'<tr><td colspan="13" class="muted">本次参数下无交易流水</td></tr>'}
function summaryHtml(d){const excess=Number(d.excess_return_pct||0),cost=d.cost_summary||{},pos=d.position_summary||{},p=btPeriod(d),params=d.params||{},metrics=d.metrics||{},combo=(d.strategy_combo_names||[]).join('、')||'未启用',low=Number(d.total_return_pct||0)<0||excess<0;const sizing=params.position_sizing||params.sizing_mode||'score_weighted';const horizon=params.horizon||'swing';return `<b>收益诊断</b><div class="summary-grid"><div><span>当前策略</span><strong class="${cls(d.total_return_pct)}">${pct(d.total_return_pct)}</strong></div><div><span>买入持有</span><strong class="${cls(d.buy_hold_return_pct)}">${pct(d.buy_hold_return_pct)}</strong></div><div><span>超额收益</span><strong class="${cls(excess)}">${pct(excess)}</strong></div><div><span>交易频率</span><strong>${esc(d.trade_count||0)}笔</strong></div><div><span>回测区间</span><strong style="font-size:13px">${esc(p.start)} 至 ${esc(p.end)}</strong></div><div><span>K线/自然日</span><strong>${esc(p.bars)} / ${esc(p.days)}</strong></div><div><span>组合策略</span><strong style="font-size:13px" title="${esc(combo)}">${esc(combo.length>20?combo.slice(0,20)+'...':combo)}</strong></div><div><span>仓位/周期</span><strong style="font-size:13px">${esc(sizing)} / ${esc(horizon)}</strong></div><div><span>复利</span><strong>${params.compound_returns===false?'关闭':'开启'}</strong></div><div><span>期望/赔率</span><strong style="font-size:13px">${esc(metrics.expectancy??'--')} / ${esc(metrics.payoff_ratio??'--')}</strong></div><div><span>均盈/均亏</span><strong style="font-size:13px">${esc(metrics.avg_win??'--')} / ${esc(metrics.avg_loss??'--')}</strong></div><div><span>连亏上限观察</span><strong>${esc(metrics.max_consecutive_losses??0)}次</strong></div><div><span>止损/止盈</span><strong style="font-size:13px">${esc(params.stop_loss_pct??'--')}% / ${esc(params.take_profit_pct??0)}%</strong></div><div><span>平均成本/股</span><strong>${esc(pos.avg_cost_basis||cost.avg_cost_basis||'--')}</strong></div><div><span>最大持仓</span><strong>${esc(pos.max_shares||cost.max_position_shares||0)}股</strong></div><div><span>期末现金/持仓</span><strong style="font-size:13px">${money(pos.cash||d.final_equity)} / ${esc(pos.shares||0)}股</strong></div></div><div class="${low?'warn':'muted'}" style="margin-top:8px">${low?'当前参数偏防守或信号滞后，可用“比较策略收益”和仓位模式继续调参。':'当前策略相对基准尚可，仍要看回撤、交易次数、成本和仓位风险。'}</div><div class="muted" style="margin-top:6px">买入/卖出已经拆成独立流水；固定止盈为0表示不开启，止损/止盈信号下一交易日开盘执行。${esc(pos.note||'')}</div>`}
