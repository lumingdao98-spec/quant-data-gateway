...
<div class="app">
  ...
  <main class="main">
    ...
    <div class="work">
      ...
      <div class="panel">
        <div class="panel-h"><span>评分与回测假设</span><div class="trade-actions"><span id="strategyName" class="pill">--</span><button class="link-btn" onclick="openTradeDrawer()">买卖明细</button><button class="link-btn" onclick="openTradePage()">新窗口</button><button class="link-btn" onclick="scrollTradeTable()">表格</button></div></div>
        <div class="panel-b">
...
function renderTradeDrawer(d){const trades=d.trades||[],cost=d.cost_summary||{},pos=d.position_summary||{},p=btPeriod(d);$('tradeDrawerSummary').innerHTML=`<div><span>标的</span><b>${esc(d.name||d.symbol)} ${esc(d.symbol)}</b></div><div><span>回测区间</span><b style="font-size:15px">${esc(p.start)} 至 ${esc(p.end)}</b></div><div><span>策略收益</span><b class="${cls(d.total_return_pct)}">${pct(d.total_return_pct)}</b></div><div><span>交易次数</span><b>${esc(d.trade_count||0)}笔</b></div><div><span>总成本</span><b>${money(cost.total_cost||0)}</b></div><div><span>成交额</span><b>${money(cost.turnover||0)}</b></div><div><span>期末现金</span><b>${money(pos.cash||d.final_equity)}</b></div><div><span>期末持仓</span><b>${esc(pos.shares||0)}股</b></div>`;$('tradeDrawerRows').innerHTML=trades.map(tradeDrawerRow).join('')||'<tr><td colspan="9" class="muted">本次参数下无闭合交易</td></tr>'}
