...
<div id="tradeDrawer" class="trade-drawer" aria-hidden="true">
  <div class="trade-drawer-panel">
    ...
    <div class="trade-drawer-body">
      ...
      <div class="table-wrap detail-table-wrap"><table><thead><tr><th>#</th><th>买入日</th><th>卖出日</th><th>持仓股数</th><th>买入价</th><th>成本/股</th><th>买入成本</th><th>卖出价</th><th>卖出金额</th><th>盈亏</th><th>收益率</th><th>买入依据</th><th>卖出依据</th></tr></thead><tbody id="tradeDrawerRows"><tr><td colspan="13" class="muted">运行回测后显示完整买卖明细</td></tr></tbody></table></div>
    </div>
  </div>
</div>
<script>
...
function tradeDrawerRow(t,i){return `<tr><td>${i+1}</td><td>${esc(t.entry_date)}</td><td>${esc(t.exit_date)}</td><td>${esc(t.buy_shares??t.shares??0)}</td><td>${esc(t.entry_price)}</td><td>${esc(t.cost_basis??'--')}</td><td>${money(t.entry_cost??t.entry_value)}</td><td>${esc(t.exit_price)}</td><td>${money(t.exit_proceeds??t.exit_value)}</td><td class="${cls(t.pnl)}">${money(t.pnl)}</td><td class="${cls(t.pnl_pct)}">${pct(t.pnl_pct)}</td><td>${esc(t.entry_reason)}<br><small class="muted">信号日 ${esc(t.entry_signal_date||'--')} · 评分 ${esc(t.entry_signal_score??'--')}</small></td><td>${esc(t.exit_reason)}<br><small class="muted">信号日 ${esc(t.exit_signal_date||'--')} · 评分 ${esc(t.exit_signal_score??'--')}</small></td></tr>`}
function renderTradeDrawer(d){const trades=d.trades||[];$('tradeDrawerSummary').innerHTML=`<div><span>标的</span><b>${esc(d.name||d.symbol)} ${esc(d.symbol)}</b></div><div><span>策略收益</span><b class="${cls(d.total_return_pct)}">${pct(d.total_return_pct)}</b></div><div><span>交易次数</span><b>${esc(d.trade_count||0)}笔</b></div><div><span>买卖/异常点</span><b>${esc((d.markers||[]).length)} / ${esc((d.anomaly_markers||[]).length)}</b></div>`;$('tradeDrawerRows').innerHTML=trades.map(tradeDrawerRow).join('')||'<tr><td colspan="13" class="muted">本次参数下无闭合交易</td></tr>'}
...
function summaryHtml(d){const excess=Number(d.excess_return_pct||0),cost=d.cost_summary||{},pos=d.position_summary||{},p=btPeriod(d),params=d.params||{},low=Number(d.total_return_pct||0)<0||excess<0;return `<b>收益诊断</b><div class="summary-grid"><div><span>当前策略</span><strong class="${cls(d.total_return_pct)}">${pct(d.total_return_pct)}</strong></div><div><span>买入持有</span><strong class="${cls(d.buy_hold_return_pct)}">${pct(d.buy_hold_return_pct)}</strong></div><div><span>超额收益</span><strong class="${cls(excess)}">${pct(excess)}</strong></div><div><span>交易频率</span><strong>${esc(d.trade_count||0)}笔</strong></div><div><span>回测区间</span><strong style="font-size:13px">${esc(p.start)} 至 ${esc(p.end)}</strong></div><div><span>K线/自然日</span><strong>${esc(p.bars)} / ${esc(p.days)}</strong></div><div><span>止损/止盈</span><strong style="font-size:13px">${esc(params.stop_loss_pct??'--')}% / ${esc(params.take_profit_pct??0)}%</strong></div><div><span>平均成本/股</span><strong>${esc(pos.avg_cost_basis||cost.avg_cost_basis||'--')}</strong></div><div><span>最大持仓</span><strong>${esc(pos.max_shares||cost.max_position_shares||0)}股</strong></div><div><span>交易成本</span><strong>${money(cost.total_cost||0)}</strong></div><div><span>佣金/滑点</span><strong style="font-size:13px">${money(cost.commission||0)} / ${money(cost.slippage_cost_est||0)}</strong></div><div><span>期末现金/持仓</span><strong style="font-size:13px">${money(pos.cash||d.final_equity)} / ${esc(pos.shares||0)}股</strong></div></div><div class="${low?'warn':'muted'}" style="margin-top:8px">${low?'当前参数偏防守或信号滞后，建议用“比较策略收益”查看是否换策略/阈值更合适。':'当前策略相对基准尚可，仍要看回撤、交易次数和成本占比。'}</div><div class="muted" style="margin-top:6px">${esc(pos.note||'')} 固定止盈为0表示不开启；止损和止盈会在收盘信号后，下一交易日开盘执行。</div>`}
function renderTradeDrawer(d){const trades=d.trades||[],cost=d.cost_summary||{},pos=d.position_summary||{},p=btPeriod(d);$('tradeDrawerSummary').innerHTML=`<div><span>标的</span><b>${esc(d.name||d.symbol)} ${esc(d.symbol)}</b></div><div><span>回测区间</span><b style="font-size:15px">${esc(p.start)} 至 ${esc(p.end)}</b></div><div><span>策略收益</span><b class="${cls(d.total_return_pct)}">${pct(d.total_return_pct)}</b></div><div><span>交易次数</span><b>${esc(d.trade_count||0)}笔</b></div><div><span>总成本</span><b>${money(cost.total_cost||0)}</b></div><div><span>成交额</span><b>${money(cost.turnover||0)}</b></div><div><span>平均成本/股</span><b>${esc(pos.avg_cost_basis||cost.avg_cost_basis||'--')}</b></div><div><span>最大持仓</span><b>${esc(pos.max_shares||cost.max_position_shares||0)}股</b></div><div><span>期末现金</span><b>${money(pos.cash||d.final_equity)}</b></div><div><span>期末持仓</span><b>${esc(pos.shares||0)}股</b></div>`;$('tradeDrawerRows').innerHTML=trades.map(tradeDrawerRow).join('')||'<tr><td colspan="13" class="muted">本次参数下无闭合交易</td></tr>'}
...
<main class="main">
  ...
  <section class="panel">
    ...
    <div class="panel-b"><div class="table-wrap"><table><thead><tr><th>#</th><th>&#20080;&#20837;&#26085;</th><th>&#21334;&#20986;&#26085;</th><th>&#25345;&#20179;&#32929;&#25968;</th><th>&#20080;&#20837;&#20215;</th><th>&#25104;&#26412;/&#32929;</th><th>&#20080;&#20837;&#25104;&#26412;</th><th>&#21334;&#20986;&#20215;</th><th>&#21334;&#20986;&#37329;&#39069;</th><th>&#30408;&#20111;</th><th>&#25910;&#30410;&#29575;</th><th>&#20080;&#20837;&#20381;&#25454;</th><th>&#21334;&#20986;&#20381;&#25454;</th></tr></thead><tbody id="rows"><tr><td colspan="13" class="muted">&#21152;&#36733;&#20013;...</td></tr></tbody></table></div></div>
  </section>
</main>
<script>
...
function row(t,i){return `<tr><td>${i+1}</td><td>${esc(t.entry_date)}</td><td>${esc(t.exit_date)}</td><td>${esc(t.buy_shares??t.shares??0)}</td><td>${esc(t.entry_price)}</td><td>${esc(t.cost_basis??'--')}</td><td>${money(t.entry_cost??t.entry_value)}</td><td>${esc(t.exit_price)}</td><td>${money(t.exit_proceeds??t.exit_value)}</td><td class="${cls(t.pnl)}">${money(t.pnl)}</td><td class="${cls(t.pnl_pct)}">${pct(t.pnl_pct)}</td><td class="reason">${esc(t.entry_reason)}<br><small class="muted">&#20449;&#21495;&#26085; ${esc(t.entry_signal_date||'--')} · &#35780;&#20998; ${esc(t.entry_signal_score??'--')}</small></td><td class="reason">${esc(t.exit_reason)}<br><small class="muted">&#20449;&#21495;&#26085; ${esc(t.exit_signal_date||'--')} · &#35780;&#20998; ${esc(t.exit_signal_score??'--')}</small></td></tr>`}
async function load(){try{const p=params();const r=await fetch('/api/backtest/run?'+p.toString(),{cache:'no-store'});const js=await r.json();if(!r.ok||!js.ok)throw new Error(js.message||('HTTP '+r.status));const d=js.data;$('sub').textContent=`${d.name} ${d.symbol} · ${d.strategy_name}`;$('mRet').textContent=pct(d.total_return_pct);$('mRet').className=cls(d.total_return_pct);$('mHold').textContent=pct(d.buy_hold_return_pct);$('mHold').className=cls(d.buy_hold_return_pct);$('mExcess').textContent=pct(d.excess_return_pct);$('mExcess').className=cls(d.excess_return_pct);$('mTrades').textContent=(d.trade_count||0)+'笔';$('mDd').textContent=pct(d.max_drawdown_pct);$('note').textContent=`${d.data_quality.start} 至 ${d.data_quality.end} · ${(d.markers||[]).length} 买卖点 · ${(d.anomaly_markers||[]).length} 异常点 · 最大持仓 ${(d.position_summary||{}).max_shares||0}股`;$('rows').innerHTML=(d.trades||[]).map(row).join('')||'<tr><td colspan="13" class="muted">&#26412;&#27425;&#21442;&#25968;&#19979;&#26080;&#38381;&#21512;&#20132;&#26131;</td></tr>'}catch(e){$('rows').innerHTML='<tr><td colspan="13" class="warn">&#21152;&#36733;&#22833;&#36133; '+esc(e)+'</td></tr>'}}
load();
</script>
</body>
</html>'''
