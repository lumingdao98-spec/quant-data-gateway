...
<div class="app">
  ...
  <main class="main">
    ...
    <div class="panel" id="tradePanel">
      ...
      <div class="table-wrap"><table><thead><tr><th>#</th><th>交易日</th><th>动作</th><th>价格</th><th>股数</th><th>成交额</th><th>费用</th><th>现金变动</th><th>现金余额</th><th>持仓股数</th><th>成本/股</th><th>已实现盈亏</th><th>交易依据</th></tr></thead><tbody id="trades"><tr><td colspan="13" class="muted">暂无交易流水</td></tr></tbody></table></div>
    </div>
  </main>
...
<div id="tradeDrawer" class="trade-drawer" aria-hidden="true">
  <div class="trade-drawer-panel">
    ...
    <div class="trade-drawer-body">
      ...
      <div class="table-wrap detail-table-wrap"><table><thead><tr><th>#</th><th>交易日</th><th>动作</th><th>价格</th><th>股数</th><th>成交额</th><th>费用</th><th>现金变动</th><th>现金余额</th><th>持仓股数</th><th>成本/股</th><th>已实现盈亏</th><th>交易依据</th></tr></thead><tbody id="tradeDrawerRows"><tr><td colspan="13" class="muted">运行回测后显示买入/卖出交易流水</td></tr></tbody></table></div>
    </div>
  </div>
</div>
<script>
...
function tradeRow(e,i){const isSell=e.side==='sell';const sideCls=e.side==='buy'?'up':'down';const pnl=isSell?money(e.realized_pnl):'--';const pnlCls=isSell?cls(e.realized_pnl):'muted';const score=e.score==null?'--':e.score;return `<tr><td>${i+1}</td><td>${esc(e.date||'--')}</td><td class="${sideCls}">${esc(e.action||e.side||'--')}</td><td>${esc(e.price??'--')}</td><td>${esc(e.shares??0)}</td><td>${money(e.amount)}</td><td>${money(e.fee)}</td><td class="${cls(e.cash_change)}">${money(e.cash_change)}</td><td>${e.cash_after==null?'--':money(e.cash_after)}</td><td>${esc(e.position_shares??'--')}</td><td>${esc(e.cost_basis??'--')}</td><td class="${pnlCls}">${pnl}${isSell?' / '+pct(e.realized_pct):''}</td><td>${esc(e.reason||'--')}<br><small class="muted">信号日 ${esc(e.signal_date||'--')} · 评分 ${esc(score)} · 闭合#${esc(e.trade_index||'--')}</small></td></tr>`}
...
function renderTradeDrawer(d){const events=tradeEvents(d),cost=d.cost_summary||{},pos=d.position_summary||{},p=btPeriod(d);$('tradeDrawerSummary').innerHTML=`<div><span>标的</span><b>${esc(d.name||d.symbol)} ${esc(d.symbol)}</b></div><div><span>回测区间</span><b style="font-size:15px">${esc(p.start)} 至 ${esc(p.end)}</b></div><div><span>策略收益</span><b class="${cls(d.total_return_pct)}">${pct(d.total_return_pct)}</b></div><div><span>闭合交易</span><b>${esc(d.trade_count||0)}笔</b></div><div><span>交易流水</span><b>${esc(d.trade_event_count||events.length)}条</b></div><div><span>总成本</span><b>${money(cost.total_cost||0)}</b></div><div><span>成交额</span><b>${money(cost.turnover||0)}</b></div><div><span>平均成本/股</span><b>${esc(pos.avg_cost_basis||cost.avg_cost_basis||'--')}</b></div><div><span>最大持仓</span><b>${esc(pos.max_shares||cost.max_position_shares||0)}股</b></div><div><span>期末现金</span><b>${money(pos.cash||d.final_equity)}</b></div><div><span>期末持仓</span><b>${esc(pos.shares||0)}股</b></div>`;$('tradeDrawerRows').innerHTML=events.map(tradeDrawerRow).join('')||'<tr><td colspan="13" class="muted">本次参数下无交易流水</td></tr>'}
...
<main class="main">
  ...
  <section class="panel">
    ...
    <div class="panel-b"><div class="table-wrap"><table><thead><tr><th>#</th><th>交易日</th><th>动作</th><th>价格</th><th>股数</th><th>成交额</th><th>费用</th><th>现金变动</th><th>现金余额</th><th>持仓股数</th><th>成本/股</th><th>已实现盈亏</th><th>交易依据</th></tr></thead><tbody id="rows"><tr><td colspan="13" class="muted">加载交易流水中...</td></tr></tbody></table></div></div>
  </section>
</main>
<script>
