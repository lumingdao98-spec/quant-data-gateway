...
const q=new URLSearchParams(location.search);if(q.get('symbol'))$('symbol').value=q.get('symbol');if(q.get('strategy'))$('strategy').value=q.get('strategy');if(q.get('strategy_combo')){const chosen=new Set(q.get('strategy_combo').split(',').filter(Boolean));document.querySelectorAll('.combo-strategy').forEach(x=>x.checked=chosen.has(x.value))}if(q.get('combo_buy_rule'))$('comboBuyRule').value=q.get('combo_buy_rule');if(q.get('combo_sell_rule'))$('comboSellRule').value=q.get('combo_sell_rule');toggleComboBox();window.addEventListener('resize',()=>{if(lastBacktest)render(lastBacktest)});if(q.get('autorun')==='1'||q.get('symbol'))runBacktest();
</script>
</body>
</html>'''
