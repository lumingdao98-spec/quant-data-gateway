...
<title>Quant Data Gateway V3.18 - ClosedLoop Cache Edition</title>
<style>
...
<div class="app">
  <div class="top">
    <div class="brand"><span class="dot"></span>量化数据网关 V3.18 · ClosedLoop Cache Edition</div>
...
<script>
const $=id=>document.getElementById(id);let rows=[],selected=null,sortKey='total_score',sortDir=-1,tableMode=localStorage.getItem('qdg_screener_view')||'compact';/* tableMode='compact' */const LS='quant_v2_watch_pool',LS_SNAPSHOT='qdg_screener_snapshot_id',LS_SELECTED='qdg_screener_selected_symbol',LS_SCROLL='qdg_screener_scroll_top',LS_PARAMS='qdg_screener_params';
...
async function runScreener(){const btn=$('runBtn');btn.disabled=true;btn.textContent='筛选中...';rows=[];render();selected=null;renderDetail(null);try{const query=buildQuery();localStorage.setItem(LS_PARAMS,query.toString());log('开始筛选，请稍候。大批量时耗时取决于公开接口和K线缓存。');const resp=await fetch('/api/screener/run?'+query.toString(),{cache:'no-store'});if(!resp.ok)throw new Error('HTTP '+resp.status);const js=await resp.json();if(!js.ok)throw new Error(js.message||'筛选失败');rows=js.data||js.results||[];if(js.screener_snapshot_id)localStorage.setItem(LS_SNAPSHOT,js.screener_snapshot_id);$('cacheHint').textContent=`缓存状态：${js.cache_status?.status||'refreshed'} · snapshot=${js.screener_snapshot_id||js.snapshot_id||'--'} · 返回筛选页会自动恢复`;log(`筛选完成：候选 ${js.result_count} / 成功分析 ${js.analyzed_count??'--'} / 股票池 ${js.pool_count??js.universe_count}，过滤 ${js.filtered_out_count??0}，耗时 ${js.elapsed_seconds}s，错误 ${js.error_count}，评分历史记录 ${js.score_history_saved??0} 条；信息面=${js.news_enabled?'启用':'未启用'}，抓取上限=${js.info_limit||'--'}，权重=${js.info_weight?Math.round(js.info_weight*100)+'%':'--'}`);if(js.errors&&js.errors.length)log('部分标的失败：'+js.errors.slice(0,5).map(e=>e.symbol+':'+e.error).join('；'),'WARN');updateMetrics(js);sortRows(false);render();const keep=localStorage.getItem(LS_SELECTED);if(keep&&rows.some(x=>x.symbol===keep))selectRow(keep);else if(rows[0])selectRow(rows[0].symbol);setTimeout(()=>{document.querySelector('.table-wrap').scrollTop=Number(localStorage.getItem(LS_SCROLL)||0)},50)}catch(e){log(e,'ERROR')}finally{btn.disabled=false;btn.textContent='开始筛选'}}
...
function clearLocalState(){localStorage.removeItem(LS_SNAPSHOT);localStorage.removeItem(LS_SELECTED);localStorage.removeItem(LS_SCROLL);localStorage.removeItem(LS_PARAMS);localStorage.removeItem('qdg_screener_view');clearResults();$('cacheHint').textContent='本地筛选状态已清空，后端缓存未删除，可从 /cache 管理。'}
async function restoreLastScreener(){try{const sid=localStorage.getItem(LS_SNAPSHOT)||'';const url=sid?'/api/screener/snapshot/'+encodeURIComponent(sid):'/api/cache/screener/latest';const resp=await fetch(url,{cache:'no-store'});const js=await resp.json();if(!js.ok){$('cacheHint').textContent='没有可恢复的筛选快照，请点击开始筛选';return}rows=js.data||js.results||js.snapshot?.results||[];if(js.snapshot_id)localStorage.setItem(LS_SNAPSHOT,js.snapshot_id);$('cacheHint').textContent=`已恢复筛选快照：${js.snapshot_id||sid||'latest'} · 缓存状态=${js.cache_status?.status||'--'}${js.cache_status?.stale?' · 已过期，可点击重新筛选':''}`;updateMetrics({result_count:rows.length,analyzed_count:js.summary?.analyzed_count,universe_count:js.summary?.result_count,error_count:js.summary?.error_count,elapsed_seconds:'cache'});render();const keep=localStorage.getItem(LS_SELECTED)||js.selected_symbol;if(keep&&rows.some(x=>x.symbol===keep))selectRow(keep);else if(rows[0])selectRow(rows[0].symbol);setTimeout(()=>{document.querySelector('.table-wrap').scrollTop=Number(localStorage.getItem(LS_SCROLL)||0)},80);log('已恢复上次筛选结果，不重新跑主流程')}catch(e){log('恢复上次筛选失败：'+e,'ERROR')}}
...
function setTableMode(mode){tableMode=mode==='debug'?'debug':mode==='full'?'full':'compact';localStorage.setItem('qdg_screener_view',tableMode);if($('compactBtn'))$('compactBtn').classList.toggle('active',tableMode==='compact');if($('fullBtn'))$('fullBtn').classList.toggle('active',tableMode==='full');if($('debugBtn'))$('debugBtn').classList.toggle('active',tableMode==='debug');render()}
...
function render(){renderHeader();const tb=document.querySelector('#resultTable tbody');if(!tb)return;const visible=tableMode==='debug'?rows:rows.filter(r=>!String(r.grade||'').startsWith('D')&&!r.excluded);tb.innerHTML=visible.length?visible.map(r=>tableMode==='compact'?compactRow(r):tableMode==='debug'?debugRow(r):fullRow(r)).join(''):`<tr><td colspan="${tableMode==='compact'?15:tableMode==='debug'?9:25}" class="muted">暂无结果，可点击“恢复上次筛选”或“开始筛选”；过期缓存不会清空。</td></tr>`}
...
function renderInfoPanel(r){
  // V3.18：筛选页展示主流程诊断字段，信息流长列表仍进入详情页，结果通过后端 snapshot 恢复。
...
(function init(){const old=localStorage.getItem(LS);if(old)$('symbols').value=old;setTableMode(tableMode);render();renderDetail(null);loadStrategyLibrary();const tw=document.querySelector('.table-wrap');if(tw)tw.addEventListener('scroll',()=>localStorage.setItem(LS_SCROLL,String(tw.scrollTop)));restoreLastScreener();log('V3.18 筛选模块初始化完成：闭环缓存、结果恢复、WordSource V2 技术因子和信息源降耗已启用')})();
</script>
</body>
</html>'''
