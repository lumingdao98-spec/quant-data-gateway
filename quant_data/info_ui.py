...
<body>
<header><h1>信息面分析详情 V3.18 / 快照复用、缓存状态、全球行业映射</h1><div><a class="toplink" href="/screener">返回筛选页</a><a class="toplink" href="/cache">缓存状态</a><a class="toplink" href="/ui">行情监控</a></div></header>
<main>
  <section class="panel">
    <div class="controls">
...
function renderSummary(){const a=state.analysis||{};const n=a.news||{};const ec=a.evidence_counts||{};$('summary').innerHTML=[
  ...
  const cs=a.cache_status||{};$('snapshotBox').innerHTML=`${a.used_snapshot||a.snapshot_reused?'当前使用筛选页/最近快照':'当前为本次刷新结果'}；快照ID：${esc(a.snapshot_id||'--')}；快照时间：${esc(a.updated_at||a.created_at||'--')}；有效证据数：${esc(ec.item_count??n.count??a.items?.length??'--')}；唯一事件数：${esc(n.count??'--')}。如需深挖，点击“深度刷新”。`;
  $('cacheStateBox').innerHTML=`缓存状态：${esc(cs.status||'--')}；年龄 ${esc(cs.age_seconds??'--')} 秒；TTL ${esc(cs.ttl_seconds??'--')} 秒；来源 ${esc(cs.source||'--')}。${cs.stale?'缓存已过期，可点击普通刷新；旧结果仍保留。':'缓存可用。'}`;
...
function renderGlobal(){const g=state.global||{};const items=g.items||[];const mapped=(state.analysis?.industry_mapped_items||[]);$('globalDims').innerHTML='<div class="small">国内 '+(g.domestic_count||0)+' / 全球 '+(g.global_count||0)+' / 商品 '+(g.commodity_count||0)+'；更新时间 '+esc(g.updated_at||'--')+'</div>'+bars(g.dimension_counts||g.category_counts||[]);$('globalSent').innerHTML=bars(g.market_category_counts||g.category_counts||[])+'<div class="small" style="margin-top:8px">情绪结构</div>'+bars([{name:'正面',count:g.positive_count||0},{name:'负面',count:g.negative_count||0},{name:'中性',count:g.neutral_count||0}], 'name','count');$('industryMappedItems').innerHTML=mapped.length?mapped.slice(0,80).map(x=>`<div class="global-card"><h4>${esc(x.title||'--')}</h4><div class="small">行业：${esc((x.mapped_industries||[]).join('、')||'--')}；概念：${esc((x.mapped_concepts||[]).join('、')||'--')}；方向：${esc(x.impact_direction||'neutral')}；相关性：${esc(x.relevance_score??'--')}；${x.score_included?'纳入个股评分':'不纳入个股评分'}</div><div class="summary-text">${esc(x.impact_reason||'')}</div></div>`).join(''):'<div class="empty">暂无与当前个股相关的全球/行业映射；不相关全球新闻只作为市场背景，不纳入个股评分。</div>';$('globalItems').innerHTML=items.length?items.slice(0,180).map(renderItem).join(''):'<div class="empty">暂无全球要闻。检查命令窗和 source_logs。</div>';}
...
(function init(){const u=new URLSearchParams(location.search);if(u.get('symbol'))$('symbol').value=u.get('symbol');if(u.get('name'))$('name').value=u.get('name');if(u.get('limit'))$('limit').value=u.get('limit'); if(u.get('snapshot_id'))log('沿用筛选页 snapshot_id='+u.get('snapshot_id')); refreshAll(u.get('force')==='true',false);setInterval(()=>{loadGlobal(false).catch(()=>{})},60000);})();
</script>
</body></html>'''
