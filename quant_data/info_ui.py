...
<header><h1>信息面分析详情 V3.18.2 / 快照复用、缓存状态、全球行业映射</h1><div><a class="toplink" href="/screener">返回筛选页</a><a class="toplink" href="/cache">缓存状态</a><a class="toplink" href="/ui">行情监控</a></div></header>
<main>
  <section class="panel">
    <div class="controls">
...
const LS_INFO_SYMBOL='quant_info_last_symbol',LS_INFO_TAB='quant_info_tab',LS_INFO_FILTERS='quant_info_filters';
function infoSnapshotKey(symbol){return 'quant_info_snapshot_id_'+String(symbol||'').trim()}
function infoItemsKey(symbol){return 'quant_info_last_items_'+String(symbol||'').trim()}
...
function renderSummary(){const a=state.analysis||{};const n=a.news||{};const ec=a.evidence_counts||{};const raw=(a.items||n.items||[]).length||n.count||0;const unknown=(a.items||n.items||[]).filter(x=>!(x.publish_time||x.published_at_norm||x.published_at||x.date)).length;$('summary').innerHTML=[
  ...
  const cs=a.cache_status||{};$('snapshotBox').innerHTML=`${a.used_snapshot||a.snapshot_reused?'Using screener/recent snapshot':'Using refreshed result'}; snapshot_id=${esc(a.snapshot_id||'--')}; created_at=${esc(a.updated_at||a.created_at||'--')}; effective=${esc(ec.item_count??n.count??a.items?.length??'--')}; unique_events=${esc(n.count??'--')}. Use deep refresh only when needed.`;
...
async function refreshAll(force=false, deep=false){const p=params();log((deep?'深度刷新':force?'强制':'快照/缓存')+`分析 ${p.name||p.symbol}，limit=${p.limit}`);const url=`/api/info/analyze/${encodeURIComponent(p.symbol)}?name=${encodeURIComponent(p.name)}&limit=${p.limit}&force=${force}&deep_refresh=${deep}&snapshot_id=${encodeURIComponent(p.snapshot_id||'')}`;const res=await fetch(url,{cache:'no-store'});const js=await res.json();if(!js.ok){log('分析失败，保留当前缓存内容');return}state.analysis=js.data;persistInfoState();renderSummary();await loadPage(1);if(deep)await loadGlobal(force);log(`分析完成：信息面分=${js.data.info_score}，新闻=${js.data.news?.count??0}条，政策线索=${js.data.policy?.policy_clue_count??0}，${js.data.snapshot_reused?'复用筛选页快照':'已刷新'}`)}
function persistInfoState(){try{const p=params();localStorage.setItem(LS_INFO_SYMBOL,p.symbol);if(state.analysis?.snapshot_id)localStorage.setItem(infoSnapshotKey(p.symbol),state.analysis.snapshot_id);localStorage.setItem(LS_INFO_TAB,state.tab||'items');localStorage.setItem(LS_INFO_FILTERS,JSON.stringify({dimension:$('dimensionFilter')?.value||'',category:$('category')?.value||'',unknown:$('unknown')?.value||'true',sort:$('sort')?.value||'desc'}));localStorage.setItem(infoItemsKey(p.symbol),JSON.stringify(state.analysis?.items||state.pageData?.data||[]))}catch(e){}}
function restoreInfoState(){try{const u=new URLSearchParams(location.search);const symbol=u.get('symbol')||localStorage.getItem(LS_INFO_SYMBOL);if(symbol)$('symbol').value=symbol;const sid=localStorage.getItem(infoSnapshotKey($('symbol').value));if(sid&&!u.get('snapshot_id'))log('本地记录最近信息快照 '+sid);const filters=JSON.parse(localStorage.getItem(LS_INFO_FILTERS)||'{}');if(filters.dimension!==undefined)$('dimensionFilter').value=filters.dimension;if(filters.category!==undefined)$('category').value=filters.category;if(filters.unknown!==undefined)$('unknown').value=filters.unknown;if(filters.sort!==undefined)$('sort').value=filters.sort;const items=JSON.parse(localStorage.getItem(infoItemsKey($('symbol').value))||'[]')||[];if(items.length){state.analysis={symbol:$('symbol').value,name:$('name').value||$('symbol').value,items,news:{count:items.length,items},source_logs:[],score_model:{formula:'local info cache'},cache_status:{status:'localStorage',source:'frontend'},diagnostics:{summary:'先显示本地信息缓存，后台自动读取快照/刷新'}};state.pageData={data:items,total:items.length,total_pages:1,stats:{}};renderSummary();renderItems();log('已先显示本地信息缓存，后台继续读取快照');}const tab=localStorage.getItem(LS_INFO_TAB);if(tab)switchTab(tab)}catch(e){log('本地信息状态恢复失败：'+e)}}
...
async function loadPage(page=1){const p=params();page=Math.max(1,page);let url=`/api/info/items/${encodeURIComponent(p.symbol)}?page=${page}&page_size=${p.page_size}&sort=${p.sort}&include_unknown_date=${p.include_unknown_date}`;if($('category').value.trim())url+='&category='+encodeURIComponent($('category').value.trim());const res=await fetch(url,{cache:'no-store'});const js=await res.json();state.pageData=js.data||{};state.total_pages=state.pageData.total_pages||1;if(page>state.total_pages&&state.total_pages>0){return loadPage(1)}state.page=page;renderItems();renderCharts();renderDedup();persistInfoState();}
...
function renderItems(){const d=state.pageData||{};const original=d.data||[];let data=original;const dim=$('dimensionFilter').value;if(dim)data=data.filter(x=>(x.message_dimension||'')===dim);const rawTotal=(state.analysis?.items||state.analysis?.news?.items||[]).length||d.total||original.length;const unknown=(state.analysis?.items||[]).filter(x=>!(x.publish_time||x.published_at_norm||x.published_at||x.date)).length;let empty='<div class="empty">No stock-specific items yet / 暂无信息. Source logs and errors remain visible; use latest snapshot, normal refresh, or deep refresh.</div>';if(!data.length&&original.length)empty='<div class="empty">Current filters hide all items. <button class="secondary" onclick="clearInfoFilters()">Clear filters</button></div>';$('items').innerHTML=data.length?data.map(renderItem).join(''):empty;$('pagerInfo').textContent=`page ${state.page} / ${state.total_pages||1}; total ${d.total??0}; raw ${rawTotal}; filtered ${data.length}; unknown_date ${unknown}; module=${$('dimensionFilter').value||'all'}; event=${$('category').value||'all'}; include_unknown_date=${$('unknown').value}`;}
...
function switchTab(t){state.tab=t;document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x.dataset.tab===t));$('tabItems').style.display=t==='items'?'block':'none';$('tabGlobal').style.display=t==='global'?'block':'none';$('tabDedup').style.display=t==='dedup'?'block':'none';$('tabFramework').style.display=t==='framework'?'block':'none';persistInfoState();}
(function init(){const u=new URLSearchParams(location.search);if(u.get('symbol'))$('symbol').value=u.get('symbol');if(u.get('name'))$('name').value=u.get('name');if(u.get('limit'))$('limit').value=u.get('limit');$('dimensionFilter').value='';$('category').value='';$('unknown').value='true';restoreInfoState();if(u.get('snapshot_id'))log('using snapshot_id='+u.get('snapshot_id')); refreshAll(u.get('force')==='true',false);setInterval(()=>{loadGlobal(false).catch(()=>{})},60000);})();
</script>
</body></html>'''
