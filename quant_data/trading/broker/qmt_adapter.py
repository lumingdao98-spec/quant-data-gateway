...
class QmtBrokerAdapter(DisabledBrokerAdapter):
    ...
    def health_check(self) -> BrokerConnectionStatus:
        if not self._supported:
            return BrokerConnectionStatus(False, "unsupported", "qmt", "本机未安装或不可导入 xtquant，QMT 适配器降级为 unsupported。")
        if not (self.config.feature_live_broker and self.config.live_trading_enabled):
            return BrokerConnectionStatus(False, "disabled", "qmt", "QMT SDK 存在，但 FEATURE_LIVE_BROKER/LIVE_TRADING_ENABLED 未开启。")
        if not self.config.qmt_account_id:
            return BrokerConnectionStatus(False, "unauthorized", "qmt", "QMT_ACCOUNT_ID 未配置。")
        return BrokerConnectionStatus(False, "not_connected", "qmt", "QMT 适配器已识别，但本轮不自动连接真实账户。")
