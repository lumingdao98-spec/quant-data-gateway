...
class NewsSnapshot:
    ...
    source: DataSourceStatus = field(default_factory=lambda: build_source_status(source_id="missing", source_name="信息源缺失", quality_status="missing"))

    def to_dict(self) -> dict[str, Any]:
        return {"symbol": self.symbol, "items": list(self.items), "source": self.source.to_dict()}


def build_news_snapshot(symbol: str, items: list[dict[str, Any]] | None, *, source_id: str = "missing", source_name: str = "") -> NewsSnapshot:
    rows = []
    missing = []
    for item in items or []:
        ...
        if row_source.quality_status == "blocked":
            ...
            continue
        rows.append({**dict(item), "source_status": row_source.to_dict()})
    if not rows:
    ...
    return NewsSnapshot(symbol=symbol, items=rows, source=source)
