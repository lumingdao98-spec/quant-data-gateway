...
class ScoreProvenanceEngine:
    ...
    def build(self, request: ScoreRequest | dict[str, Any]) -> ScoreProvenanceV323:
        ...
        for key, weight in policy.dimension_weights.items():
            raw = float(req.factor_values.get(key, 50.0) if req.factor_values.get(key, 50.0) is not None else 50.0)
            contribution = raw * weight if weight >= 0 else (100.0 - raw) * abs(weight)
            final += contribution
            ...
            contributions.append(
                FactorContribution(
                    ...
                    explanation=_explain_dimension(key, raw, weight),
                    dimension=key,
                )
            )
