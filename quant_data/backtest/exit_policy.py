    ...
    def evaluate(
        ...
        *,
        ...
        if config.atr_stop_multiplier and atr_pct and pnl_pct <= -abs(config.atr_stop_multiplier * atr_pct):
            return ExitDecision(True, "atr_stop_loss", f"ATR止损 {pnl_pct:.2f}% / ATR {atr_pct:.2f}%")
        if config.trailing_stop_pct and from_high_pct <= -abs(config.trailing_stop_pct):
            return ExitDecision(True, "trailing_stop", f"跟踪止损 {from_high_pct:.2f}%")
        if ma20 and close < ma20:
            return ExitDecision(True, "ma20_break", "跌破MA20")
        if behavior_risk_reversal:
            return ExitDecision(True, "behavior_risk_reversal", "行为风险反转")
        if config.max_holding_days and bars_held >= config.max_holding_days and pnl_pct <= 0:
            return ExitDecision(True, "max_holding_no_profit", "持有超时且无盈利")
        if config.take_profit_pct and pnl_pct >= abs(config.take_profit_pct):
            return ExitDecision(True, "target_profit", f"目标止盈 {pnl_pct:.2f}%")
        if config.partial_take_profit_pct and pnl_pct >= abs(config.partial_take_profit_pct):
            return ExitDecision(True, "partial_take_profit", f"分批止盈 {pnl_pct:.2f}%", partial_pct=0.5)
        if score is not None and score < config.sell_score:
            return ExitDecision(True, "score_exit", f"评分跌破卖出线 {score:.1f} < {config.sell_score:.1f}")
        if sector_retreat:
            return ExitDecision(True, "sector_retreat", "板块强度回落")
        return ExitDecision(False)
